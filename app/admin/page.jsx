...
export default function AdminPage() {
  ...
  if (!user) return <main className="flex min-h-screen items-center justify-center bg-[#07111f] px-6 py-28 text-white"><form onSubmit={handleLogin} className="w-full max-w-md rounded-3xl border border-slate-700 bg-[#0d1b2e] p-8 shadow-2xl"><p className="text-sm font-semibold uppercase tracking-[0.25em] text-cyan-300">MSI.dev studio</p><h1 className="mt-3 text-3xl font-bold">Your site, in your hands.</h1><p className="mt-3 text-sm leading-6 text-slate-400">Sign in to edit every visible section and publish media without touching code.</p><div className="mt-8 space-y-4"><input className={inputClass} aria-label="Email" type="email" placeholder="Email" value={login.email} onChange={(event) => setLogin({ ...login, email: event.target.value })} required /><input className={inputClass} aria-label="Password" type="password" placeholder="Password" value={login.password} onChange={(event) => setLogin({ ...login, password: event.target.value })} required /></div><button disabled={busy} className="mt-6 flex w-full items-center justify-center gap-2 rounded-xl bg-cyan-300 px-4 py-3 font-semibold text-[#07111f] hover:bg-cyan-200 disabled:opacity-50"><LogIn size={17} /> {busy ? "Signing in..." : "Sign in"}</button><p className="mt-5 text-xs text-slate-500">Local demo: admin@msi.dev / admin123</p>{notice && <p className="mt-4 text-sm text-amber-300">{notice}</p>}</form></main>;

  if (loadingContent) return <main className="flex min-h-screen items-center justify-center bg-[#07111f] px-6 py-28 text-white"><div className="rounded-2xl border border-slate-700 bg-[#0d1b2e] px-8 py-6 text-center"><p className="font-semibold text-cyan-300">Loading your content</p><p className="mt-2 text-sm text-slate-400">This will stop automatically if Firebase is unreachable.</p></div></main>;

  return <main className="min-h-screen bg-[#07111f] px-6 py-28 text-white sm:px-12 lg:px-20"><div className="mx-auto max-w-7xl"><header className="flex flex-wrap items-end justify-between gap-5 border-b border-slate-700 pb-8"><div><p className="text-sm font-semibold uppercase tracking-[0.25em] text-cyan-300">MSI.dev studio</p><h1 className="mt-3 text-4xl font-bold">Edit your whole site</h1><p className="mt-3 text-slate-400">{hasFirebaseConfig ? "Connected to Firebase" : "Local preview mode"}</p></div><div className="flex flex-wrap gap-3"><button type="button" disabled={busy} onClick={handleSave} className="inline-flex items-center gap-2 rounded-xl bg-cyan-300 px-4 py-3 font-semibold text-[#07111f] disabled:opacity-50"><Save size={17} /> {busy ? "Working..." : "Publish changes"}</button><button type="button" onClick={() => { if (hasFirebaseConfig) signOut(auth); else { window.sessionStorage.removeItem("msi-admin-session"); setUser(null); } }} className="inline-flex items-center gap-2 rounded-xl border border-slate-700 px-4 py-3 text-slate-300"><LogOut size={17} /> Log out</button></div></header>{notice && <p className="mt-6 rounded-xl border border-cyan-300/30 bg-cyan-300/10 p-4 text-cyan-200">{notice}</p>}
      <div className="mt-10 grid gap-8 xl:grid-cols-2">
        ...
        <EditorSection title={`Messages (${messages.filter((message) => message.status === "unread").length} unread)`}>{messages.length === 0 ? <p className="text-sm text-slate-400">No messages yet.</p> : <div className="space-y-3">{messages.map((message) => <article key={message.id} onClick={() => handleMessageOpen(message)} className={`rounded-xl border p-4 ${message.status === "unread" ? "border-cyan-300/60 bg-cyan-300/5" : "border-slate-700"}`}><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-semibold">{message.subject}</p><p className="mt-1 text-sm text-cyan-200">{message.name} · {message.email}</p></div><div className="flex items-center gap-3"><time className="text-xs text-slate-500">{new Date(message.createdAt).toLocaleString()}</time><button type="button" aria-label={`Delete message from ${message.name}`} onClick={async (event) => { event.stopPropagation(); await deleteMessage(message.id); setMessages((current) => current.filter((item) => item.id !== message.id)); setNotice("Message deleted."); }} className="text-slate-400 hover:text-red-300"><Trash2 size={16} /></button></div></div><p className="mt-3 whitespace-pre-wrap text-sm leading-6 text-slate-300">{message.message}</p><a href={`mailto:${message.email}?subject=${encodeURIComponent(`Re: ${message.subject}`)}`} onClick={() => handleMessageOpen(message)} className="mt-4 inline-block text-sm font-semibold text-cyan-300 hover:text-cyan-200">Reply by email</a></article>)}</div>}</EditorSection>
      </div></div></main>;
}

function EditorSection({ title, children }) { return <section className="rounded-3xl border border-slate-700 bg-[#0d1b2e] p-6"><h2 className="mb-5 text-xl font-semibold text-cyan-300">{title}</h2><div className="space-y-4">{children}</div></section>; }
